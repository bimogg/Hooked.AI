...
export default function HookPlayer({ thumbnailUrl, reelUrl }: Props) {
  ...
  return (
    <div className="relative w-full h-full bg-black">
      ...
      {/* iframe preloads in background from the start */}
      {embedUrl && (
        <iframe
          src={embedUrl}
      ...
      {/* Thumbnail overlay — removed instantly on click, iframe already loaded */}
      {!active && (
        <div
      ...
      <button
        onClick={openReel}
        className="absolute top-2 right-2 w-7 h-7 bg-black/50 hover:bg-black rounded-full flex items-center justify-center transition-colors z-20"
